...
* All Rights Reserved
*----------------------------------------------------------------------*
* Application : Nhập xuất tồn số lượng giá trị
* Creation Date: Jan 3, 2024 4:49:57 PM
* Created by: NganNM
*----------------------------------------------------------------------*
class zwm_cl_nxt_slgt_ch definition
  public
  final
  create public .
  ...
  public section.
    ...
    data:
        gt_data type table of zwm_i_nxt_slgt_ch.
  protected section.
  private section.
...
CLASS ZWM_CL_NXT_SLGT_CH IMPLEMENTATION.
  ...
  method get_data.
   ...
   where storagelocation in @gr_storagelocation
     and material in @gr_material
     and plant in @gr_plant
     and batch in @gr_batch
     and companycode in @gr_bukrs
   group by periodtype,
            material,
            plant,
            storagelocation,
   ...
   into table @data(lt_currentstock).
    ...
    select
        ...
        i_product~yy1_materialfast_prd,
  ...
  method get_tabname.
    ...
    case lv_field.
      ...
      when 'YY1_MATERIALFAST_PRD'.
        tabname = 'i_product~yy1_materialfast_prd'.
